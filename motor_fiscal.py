...
def gerar_excel_final(df_ent, df_sai, file_ger_ent=None, file_ger_sai=None):
    ...
    if not df_pc_audit.empty: df_pc_audit[['Diagnóstico', 'CST XML (P/C)', 'CST Esperado (P/C)', 'Ação']] = df_pc_audit.apply(audit_pc_xml, axis=1)

    df_ipi_audit = df_sai.copy()
    def audit_ipi_xml(row):
        ncm = str(row['NCM']).zfill(8); info = base_pc[base_pc['NCM_KEY'] == ncm] if not base_pc.empty else pd.DataFrame()
    ...
    if not df_ipi_audit.empty: df_ipi_audit[['Diagnóstico', 'CST XML', 'CST Base', 'IPI XML', 'IPI Esperado', 'Ação', 'Complemento']] = df_ipi_audit.apply(audit_ipi_xml, axis=1)

    df_difal_audit = df_sai.copy()
    def audit_difal_xml(row):
    ...
    if not df_difal_audit.empty: df_difal_audit[['Diagnóstico', 'DIFAL XML', 'Ação']] = df_difal_audit.apply(audit_difal_xml, axis=1)
    ...
    # --- NOVA ABA: PIS e COFINS (Padrão Escritório) ---
    ...
    df_apuracao_final = gerar_apuracao_pc_escritorio(df_ge, df_gs)

    # --- GRAVAÇÃO FINAL (MANTENDO TUDO) ---
    mem = io.BytesIO()
